...
st.set_page_config(
    page_title="Malaria incidence alert",
    page_icon=":sparkles:",
    layout="wide",
    initial_sidebar_state="expanded"
)

st.markdown("<span style='color:red; font-size:40px;'>Application pour l'alerte d'épidémie du paludisme</span>", unsafe_allow_html=True)
#st.markdown('''
## Application pour l'alerte d'épidémie du paludisme
#''')
#st.subheader("le risque de l'épidémie du paludisme vous est prédit")
...
def user_input():
    input_var1=st.sidebar.slider('Précipitation',0,500,500)
    input_var2=st.sidebar.slider('Température maximale moyenne',29,11,40)
    input_var3=st.sidebar.slider('Humidité relative maximale',50,50,100)
    data ={'Prec_Average': input_var1,
           'Average_Temperature_Max': input_var2,
           'Average_RH_Max': input_var3}
    input_data=pd.DataFrame(data,index=[0])
...
df = user_input()
prediction =svr_model.predict(df)
st.sidebar.write("Incidence du Paludisme actuel:","{:.2f}".format(prediction[0]),"%")
if(prediction[0]>= 0 and prediction[0]<15):
    st.sidebar.markdown('Statut: <span style="color:green;">Faible</span>', unsafe_allow_html=True)
elif(prediction[0]>= 15 and prediction[0]<45):
    st.sidebar.markdown('Statut: <span style="color:yellow;">Modéré</span>', unsafe_allow_html=True)
elif(prediction[0]>= 45 and prediction[0]<75):
    st.sidebar.markdown('Statut: <span style="color:orange;">Elevé</span>', unsafe_allow_html=True)
else:
    st.sidebar.markdown('Statut: <span style="color:red;"> Très élevé</span>', unsafe_allow_html=True)
...
with col1:
    ...
    if auto_df.isnull().values.any():
        st.markdown('<span style="color:red;">Echec de Prédiction</span>', unsafe_allow_html=True)
        st.markdown('<span style="color:red;">Données indisponibles, essayez une ville proche</span>', unsafe_allow_html=True)

    else:
        # st.dataframe()
        auto_prediction = svr_model.predict(auto_df)
        st.write("L'incidence du paludisme est de:", "{:.2f}".format(auto_prediction[0]), "%")
    # Interface in the right column
with col2:
    st.markdown("#### Comment ça marche?")
    st.write("Renseignez les données d'entrées en glissant les curseurs")
    st.write("OU")
    #st.write("Données courantes extraites de Meteostat")
    st.write("Choisissez une ville pour une prédiction en temps réel")


st.write()
>>>>>>> 0455128 (Premier commit)
